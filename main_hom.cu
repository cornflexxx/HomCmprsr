...
int main() {
  ...
  */
  ...
  quant_prediction_local<<<grid, block>>>(d_localData, d_quantLocOut, eb,
                                          nbEle);
  cudaDeviceSynchronize();
  int *quantLocOut = (int *)malloc(nbEle * sizeof(int));
  cudaMemcpy(quantLocOut, d_quantLocOut, nbEle * sizeof(int),
             cudaMemcpyDeviceToHost);
  write_datai("outQuant", quantLocOut, nbEle);

  cudaStream_t stream, streamDecompr;
  cudaStreamCreate(&stream);
  cudaStreamCreate(&streamDecompr);
  ...
  if (err != cudaSuccess) {
    fprintf(stderr, "Error in homomorphic_sum: %s\n", cudaGetErrorString(err));
  }
